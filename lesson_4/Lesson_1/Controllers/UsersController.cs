...
namespace Lesson1_login.Controllers
{
    ...
    public class UsersController : ControllerBase
    {
        ...
        {
            User1 user =await  _usersService.GetUserByIdAsync(id);
        ...
        [HttpPost]
        public async Task<ActionResult<User1>> Post([FromBody] User1 user)
        {
            Password pass = new Password(user.Password);
            if(_passwordsService.getPasswordRate(pass)>2)
            {
                User1 userCreated =await _usersService.CreateUserAsync(user);
                if (userCreated != null)
       ...
       public async Task<ActionResult<User1>> SignIn([FromBody] User1 data)
        {
            User1 user =await _usersService.SignInAsync(data);
            return user == null ? NotFound() : user;
      ...
      //  PUT api/<LoginController>
        ...
        [HttpPut("{id}")]
        public async Task Put(int id, [FromBody] User1 userToUpdate)
        {
